				...
				if !additionalBuildContext.IsImage {
					...
					if additionalBuildContext.IsURL {
						...
						if additionalBuildContext.DownloadedCache == "" {
							...
							path, subdir, err := tmpdirpkg.ForURL(s.ctx, tmpdir.GetTempDir(), internal.BuildahExternalArtifactsDir, additionalBuildContext.Value, urlOptions)
							if err != nil {
		...
		for _, flag := range node.Flags {
			...
			if mountOptionFrom != "" {
				if otherStageIndex, _ := s.executor.stageIndex(mountOptionFrom, s.stages[:s.stage.Position]); otherStageIndex != -1 {
					...
					if image, isPreviousStage := s.executor.imageDigestMap[otherStageIndex]; isPreviousStage {
						mountCheckSum = image
					}
				} else {
