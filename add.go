...
type AddAndCopyOptions struct {
	...
	Hasher io.Writer
	// Excludes contents in the .dockerignore file.
	Excludes []string
	// ContextDir is the current root directory on the host.
	ContextDir string
	// DryRun indicates that the content should be read for hashing, but
	// not actually written to the container.
	DryRun bool
}
...
func addURL(destination, srcurl string, owner idtools.IDPair, hasher io.Writer, dryRun bool) error {
	...
	if hasher != nil {
		...
		if modifiedTime != nil {
			hdr.ModTime = *modifiedTime
		}
		if err := tar.NewWriter(hasher).WriteHeader(&hdr); err != nil {
			return errors.Wrapf(err, "error digesting header information for %q", destination)
		}
...
func (b *Builder) Add(destination string, extract bool, options AddAndCopyOptions, source ...string) error {
	...
	copyFileWithTar := b.copyFileWithTar(&containerOwner, options.Hasher, options.DryRun)
	copyWithTar := b.copyWithTar(&containerOwner, options.Excludes, options.Hasher, options.DryRun)
