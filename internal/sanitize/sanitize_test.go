...
func TestSanitizeImageName(t *testing.T) {
	...
	newGoodLayout, nameless, err := ImageName(store, "oci", goodLayoutRel, contextDir, t.TempDir())
	require.NoError(t, err, "sanitizing good OCI layout")
	require.True(t, nameless, "should be discarding names of oci layout locations")
	...
	newGoodDir, nameless, err := ImageName(store, "dir", goodDirRel, contextDir, t.TempDir())
	require.True(t, nameless, "should be discarding names of dir locations")
	...
	newGoodOCIArchive, nameless, err := ImageName(store, "oci-archive", goodOCIArchiveRel, contextDir, t.TempDir())
	require.NoError(t, err, "sanitizing good OCI archive")
	require.True(t, nameless, "should be discarding names of oci archives")
	...
	newGoodDockerArchive, nameless, err := ImageName(store, "docker-archive", goodDockerArchiveRel, contextDir, t.TempDir())
	require.NoError(t, err, "sanitizing good docker archive")
	require.True(t, nameless, "should be discarding names of docker archives")
	...
	_, _, err = ImageName(store, "oci", badLayoutRel, contextDir, t.TempDir())
	...
	_, _, err = ImageName(store, "dir", badDirRel, contextDir, t.TempDir())
	...
	_, _, err = ImageName(store, "oci-archive", badOCIArchiveRel, contextDir, t.TempDir())
	...
	_, _, err = ImageName(store, "docker-archive", badDockerArchiveRel, contextDir, t.TempDir())
